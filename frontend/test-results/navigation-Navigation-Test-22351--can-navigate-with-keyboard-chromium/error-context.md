...
- Name: navigation.spec.ts >> Navigation Tests >> Keyboard Navigation >> can navigate with keyboard
- Location: e2e/navigation.spec.ts:106:5
  ...
  - generic [ref=e10]:
    ...
    - list [ref=e49]:
      ...
      - listitem [ref=e59]:
        ...
        - text: "|"
      - listitem [ref=e61]:
        - link "Grafana v12.4.0 (d1729c53a7)" [ref=e62] [cursor=pointer]:
          - /url: https://github.com/grafana/grafana/blob/main/CHANGELOG.md
        - text: "|"
      - listitem [ref=e63]:
        - img [ref=e64]
        - link "New version available!" [ref=e65] [cursor=pointer]:
...
> 110 |       expect(await focused.count()).toBeGreaterThan(0);
  ...
  124 |       await page.goto(`${BASE_URL}/documents`);
  125 |       await expect(page).toHaveURL(/documents/);
  126 |     });
  127 | 
  128 |     test('unknown route shows 404', async ({ page }) => {
  129 |       await page.goto(`${BASE_URL}/nonexistent-page-12345`);
  130 |       const error = page.locator('text=404,Not Found,Pronađeno').first();
  131 |       const hasError = await error.isVisible().catch(() => false);
  132 |       expect(hasError || page.url().includes('nonexistent')).toBe(true);
  133 |     });
  134 | 
  135 |     test('redirects work correctly', async ({ page }) => {
  136 |       await page.goto(BASE_URL);
  137 |       await expect(page).not.toHaveURL(/redirect/);
  138 |     });
  139 |   });
  140 | 
  141 |   test.describe('Back Button', () => {
  142 |     test('browser back button works', async ({ page }) => {
  143 |       await page.goto(`${BASE_URL}/documents`);
  144 |       await page.goto(`${BASE_URL}/quizzes`);
  145 |       await page.goBack();
  146 |       await expect(page).toHaveURL(/documents/);
  147 |     });
  148 | 
  149 |     test('browser forward button works', async ({ page }) => {
  150 |       await page.goto(`${BASE_URL}/documents`);
  151 |       await page.goto(`${BASE_URL}/quizzes`);
  152 |       await page.goBack();
  153 |       await page.goForward();
  154 |       await expect(page).toHaveURL(/quizzes/);
  155 |     });
  156 |   });
  157 | });
```
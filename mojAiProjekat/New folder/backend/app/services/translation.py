...
def make_gemini_client(api_key: str):
    """Kreira Gemini translation client."""
    class GeminiTranslationClient:
        def __init__(self, key):
            self.key = key
        def translate(self, text, source, target):
            return translation_service.translate(text, source, target, provider="gemini")
    return GeminiTranslationClient(api_key)


def make_groq_client(api_key: str):
    """Kreira Groq translation client (koristi OpenAI compat API)."""
...
def make_mistral_client(api_key: str):
    """Kreira Mistral translation client."""
    class MistralTranslationClient:
        def __init__(self, key):
            self.key = key
        def translate(self, text, source, target):
            return translation_service.translate(text, source, target, provider="mistral")

...
class PDFService:
    ...
    def __init__(
        ...
        if TIKTOKEN_AVAILABLE:
            try:
                ...
                self.encoder = None
                logger.warning("Tiktoken encoder not available, using word count approximation")
        else:
            self.encoder = None
    ...
    def perform_ocr(self, pdf_bytes: bytes, page_numbers: List[int] = None, progress_callback=None) -> Dict[int, str]:
        """
    ...
    def process_pdf(
        ...
        try:
            ...
            if ocr_needed and self.use_ocr:
                ...
                ocr_results = self.perform_ocr(pdf_bytes, progress_callback=ocr_progress_wrapper)

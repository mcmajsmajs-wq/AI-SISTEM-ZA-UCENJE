# -*- coding: utf-8 -*-
"""
Knowledge Ingestion servis.

Obrađuje različite izvore podataka i priprema ih za RAG:
- PDF fajlovi (ekstrahuje tekst)
- Markdown fajlovi
- Plain text
- Web stranice (HTTP scraping)
- Aplikacijski logovi
"""
...
def extract_text_from_pdf(file_path: str) -> str:
    """Ekstrahuje tekst iz PDF fajla koristeći PyMuPDF."""
    try:
        import fitz  # PyMuPDF
...
def extract_text_from_markdown(file_path: str) -> str:
    """Čita Markdown fajl i vraća tekst (uklanja Markdown sintaksu)."""
    try:
...
def extract_errors_from_logs(log_dir: str, max_lines: int = 500) -> str:
    ...
    lines = []
    for log_file in sorted(log_path.glob("*.log"), key=lambda f: f.stat().st_mtime, reverse=True)[:3]:
        try:
            content = log_file.read_text(encoding="utf-8", errors="ignore")
            relevant = [
                line for line in content.split("\n")
                if any(kw in line.upper() for kw in ["ERROR", "WARNING", "CRITICAL", "EXCEPTION"])
            ]
...
def ingest_source(db, source_id: str, source_type: str, content: str, name: str) -> int:
    ...
    if not content or not content.strip():
        ...
        db.execute(text("UPDATE knowledge_sources SET status = 'error', updated_at = NOW() WHERE id = :id"), {"id": source_id})
        db.commit()
        return 0

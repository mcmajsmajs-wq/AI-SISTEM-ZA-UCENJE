...
================================================================================
Jedinstveni AI client koji objedinjuje sve provajdere:
- OpenAI (GPT-4o, o1)
- Claude (Anthropic)
- DeepSeek
- Gemini (Google)
...
Podržava:
- Multi-turn chat sa kontekstom
- Tool calling (funkcijski pozivi)
- Thinking mode (prošireno rezonovanje)
- Streaming odgovore
...
class BaseAIChatClient(ABC):
    """Bazna klasa za AI chat klijente."""
    ...
    async def chat(
        ...
        **kwargs
        ...
        """Pošalji chat poruke i dobij odgovor."""
        pass
    ...
    async def chat_stream(
        ...
        **kwargs
        ...
        """Pošalji chat poruke sa streaming odgovorom."""
        pass
    
    @abstractmethod
    def is_available(self) -> bool:
        """Proveri da li je provajder dostupan."""
        pass
...
class OpenAIChatClient(BaseAIChatClient):
    ...
    def _convert_messages(self, messages: List[ChatMessage]) -> List[Dict]:
        """Konvertuj ChatMessage u OpenAI format."""
        result = []
        for msg in messages:
    ...
    async def chat(
        ...
        **kwargs
        ...
        if not self.api_key:
            return ChatResponse(
                content="OpenAI API key nije konfigurisan",
...
class ClaudeChatClient(BaseAIChatClient):
    ...
    async def chat(
        ...
        **kwargs
        ...
        if not self.api_key:
            return ChatResponse(
                content="Claude API key nije konfigurisan",
                provider=self.provider_name
            )
...
class GeminiChatClient(BaseAIChatClient):
    ...
    async def chat(
        ...
        **kwargs
        ...
        if not self.api_key:
            return ChatResponse(
                content="Gemini API key nije konfigurisan",
                provider=self.provider_name
            )
...
class AIChatService:
    ...
    def get_available_providers(self) -> List[ProviderInfo]:
        """Vrati listu dostupnih provajdera."""
        providers = []
        
        provider_configs = [
            (AIProvider.OPENAI.value, "OpenAI", "gpt-4o", True, True),
            (AIProvider.CLAUDE.value, "Claude", "claude-3-5-sonnet-20241022", True, True),
    ...
    def _get_client(self, provider: str) -> Optional[BaseAIChatClient]:
        """Dobij klijenta za datog provajdera."""
    ...
    async def chat(
        ...
        if not client:
            return ChatResponse(
                content="Nijedan AI provajder nije dostupan. Proverite API ključeve.",
                provider=provider
            )
...
def get_ai_service(user_api_keys: Optional[Dict[str, str]] = None) -> AIChatService:
    """Factory function za AIChatService."""

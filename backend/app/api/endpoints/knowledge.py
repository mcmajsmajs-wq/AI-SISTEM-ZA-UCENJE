...
async def query_knowledge(
    ...
    result = await rag_query(db, body.query, user=current_user, top_k=body.top_k, provider_override=body.provider)
...
def list_sources(
    ...
    return [
        SourceResponse(
            id=r.id, source_type=r.source_type, name=r.name, url=r.url,
            total_chunks=r.total_chunks, status=r.status,
            last_indexed=r.last_indexed, created_at=r.created_at
        )
        for r in rows
    ]
...
def knowledge_stats(
    ...
    return {
        ...
        "by_type": [{"type": r.source_type, "count": r.count, "chunks": r.chunks} for r in by_type],
    }
...
def ingest_url(
    ...
    if existing and existing.status == 'indexed':
        return {"message": "URL već indeksiran", "source_id": str(existing.id), "status": "exists"}
    ...
    if body.recursive:
        ...
        return {"message": "URL dodat u red za indeksiranje", "task_id": None, "status": "queued"}
...
def _run_crawl_site(url: str, max_depth: int, max_pages: int, source_name: Optional[str], created_by: Optional[str]):
    ...
    crawl_site_task.apply(
        kwargs={"start_url": url, "max_depth": max_depth, "max_pages": max_pages,
                "source_name": source_name, "created_by": created_by}
    )
...
def delete_source(
    ...
    existing = db.execute(
        text("SELECT id FROM knowledge_sources WHERE id = :id"),
        {"id": source_id}
    ).fetchone()

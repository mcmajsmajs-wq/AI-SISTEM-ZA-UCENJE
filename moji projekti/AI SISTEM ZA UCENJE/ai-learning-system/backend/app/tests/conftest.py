...
def test_document(db: Session, test_file: File, test_document_data: dict) -> Document:
    ...
    document = Document(
        ...
        author=test_document_data["author"],
...
def test_chunks(db: Session, test_document: Document) -> list:
    ...
    for i in range(5):
        chunk = Chunk(
            ...
            chunk_index=i,
            original_text=f"This is test chunk number {i + 1}. It contains some English text for translation testing.",
            translated_text=f"Ovo je test odlomak broj {i + 1}. Sadrzi neki engleski tekst za testiranje prevoda." if i < 3 else None,
            status="translated" if i < 3 else "pending",
            page_number=i + 1,
            heading=f"Section {i + 1}" if i % 2 == 0 else None,
            is_edited=False,
            created_at=datetime.utcnow(),
            updated_at=datetime.utcnow()
        )

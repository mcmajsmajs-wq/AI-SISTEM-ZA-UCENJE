...
class TestTranslationProvider:
    ...
    def test_provider_count(self):
        ...
        assert len(TranslationProvider) == 5
...
class TestGoogleTranslateClient:
    ...
    def test_translate_success(self):
        ...
        with patch('httpx.post') as mock_post:
            ...
            mock_response.json.return_value = {
                "data": {
                    "translations": [{"translatedText": "Ovo je prevod"}]
                }
            }
            mock_post.return_value = mock_response
...
class TestTranslationService:
    ...
    def test_translate_with_specific_provider(self):
        ...
        with patch.object(service._clients['ollama'], 'is_available', return_value=True):
            with patch.object(service._clients['ollama'], 'translate') as mock_translate:
                mock_translate.return_value = TranslationResult(
                    success=True,
                    translated_text="Prevod",
                    provider="ollama"
                )
    ...
    def test_translate_fallback(self):
        ...
        with patch.object(service._clients['ollama'], 'is_available', return_value=True):
            with patch.object(service._clients['ollama'], 'translate') as mock_ollama:
                ...
                with patch.object(service._clients['deepl'], 'is_available', return_value=True):
                    with patch.object(service._clients['deepl'], 'translate') as mock_deepl:
    ...
    def test_translate_all_providers_fail(self):
        ...
        for client in service._clients.values():
            with patch.object(client, 'is_available', return_value=True):
                with patch.object(client, 'translate', return_value=TranslationResult(
                    success=False,
                    error="Failed"
                )):
                    result = service.translate("Test")
    ...
    def test_translate_batch(self):
        ...
        with patch.object(service, 'translate') as mock_translate:
            mock_translate.side_effect = [
                TranslationResult(success=True, translated_text="Prevod 1", tokens_used=10),
                TranslationResult(success=True, translated_text="Prevod 2", tokens_used=20),
                TranslationResult(success=True, translated_text="Prevod 3", tokens_used=30),
            ]
    ...
    def test_glossary_applied_to_translation(self):
        ...
        with patch.object(service._clients['ollama'], 'is_available', return_value=True):
            with patch.object(service._clients['ollama'], 'translate') as mock_translate:
                mock_translate.return_value = TranslationResult(
                    success=True,
                    translated_text="AI je važna tehnologija"
                )
    ...
    def test_provider_not_available(self):
        ...
        with patch.object(service._clients['deepl'], 'is_available', return_value=False):
...
class TestEdgeCases:
    ...
    def test_special_characters(self):
        ...
        with patch.object(service, 'translate') as mock_translate:
            mock_translate.return_value = TranslationResult(
                success=True,
                translated_text="Zdravo! @#$%^&*() 你好 مرحبا"
            )

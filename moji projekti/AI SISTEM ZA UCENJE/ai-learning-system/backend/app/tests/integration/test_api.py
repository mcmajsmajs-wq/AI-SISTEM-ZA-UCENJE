...
class TestUserEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_change_password_success(self, db, test_user, test_user_token, test_user_data):
...
class TestDocumentEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_get_document_by_id(self, db, test_user, test_user_token, test_document):
    ...
    @pytest.mark.asyncio
    async def test_get_document_chunks(self, db, test_user, test_user_token, test_document, test_chunks):
    ...
    @pytest.mark.asyncio
    async def test_update_chunk(self, db, test_user, test_user_token, test_document, test_chunks):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            response = await client.put(
                ...
                headers={"Authorization": f"Bearer {test_user_token}"},
                json={
                    "translated_text": "Izmenjeni prevod teksta."
                }
            )
        
        assert response.status_code == 200
        data = response.json()
        assert data["translated_text"] == "Izmenjeni prevod teksta."
